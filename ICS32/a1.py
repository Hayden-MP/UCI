...
def c_command(query):
    path = query[1]                             # Get path from query
...
def parse(input_parts: list):
    ...
    query.append(command)               # Store command into query[0]
    if(path == ' '):                    # If path is just space,
        query.append('')                # path was not given, query[1] = ''
    else:
        query.append(Path(path.strip()))    # Else, insert path into query[1]
    query.append(options)                   # List of options in query[2]
    query.append(input2.strip())            # Second input (filenames, etc) in query[3]

    # Last refinements of query 
    if(str(query[1]) == query[3]):      # If no options are given, my parser
        query.remove(query[3])          # doesn't know where path starts so
        query.insert(3,'')              # input2 is the actual path, so I remove
                                        # from query[3] if they are equal
    if(query[3] == '/'):            
        query[3] = ''                   # If input2 is just a /, that means its
                                        # empty. I replace it here
    return query
